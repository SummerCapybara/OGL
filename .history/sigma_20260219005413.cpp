...
int main() {
	...
	while (!glfwWindowShouldClose(engine.window)) {
		...
		static int verticesinthecircle = 3;
		if (glfwGetKey(engine.window, GLFW_KEY_EQUAL)) {
			verticesinthecircle++;
			vertices = genCircle(verticesinthecircle, accum);
			glBindBuffer(GL_ARRAY_BUFFER, VBO);
			glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(glm::vec2),
			             vertices.data(), GL_STATIC_DRAW);
		}
		if (HandleInput(engine.window, GLFW_KEY_APOSTROPHE) {
			accum += 10.0f;
			vertices = genCircl(verticesinthecircle, accum);
			glBindBuffer(GL_ARRAY_BUFFER, VBO);
			glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(glm::vec2), vertices.data(), GL_STATIC_SR)
		})
		// if (glfwGetKey(engine.window, GLFW_KEY_EQUAL) == GLFW_PRESS) {
		// 	if (!pressedEq) {
		// 		pressedEq = true;
		// 		verticesinthecircle++;
		// 		vertices = genCircle(verticesinthecircle, accum);
		...
		// sizeof(glm::vec2), vertices.data(), GL_STATIC_DRAW);
		// 	}
		// }
		// if (glfwGetKey(engine.window, GLFW_KEY_EQUAL) == GLFW_RELEASE) {
		// 	if (pressedEq) {
		// 		pressedEq = false;
		// 	}
		// }
		static bool pressedMi = false;
		if (glfwGetKey(engine.window, GLFW_KEY_MINUS) == GLFW_PRESS) {
			if (!pressedMi) {
				pressedMi = true;
				verticesinthecircle--;
				vertices = genCircle(verticesinthecircle, accum);
		...
		if (glfwGetKey(engine.window, GLFW_KEY_MINUS) == GLFW_RELEASE) {
			if (pressedMi) {
				pressedMi = false;
			}
		}
